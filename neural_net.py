...
#    print(np.argmax(a3, axis = 1) [:,np.newaxis])
    print(np.round(a3[0]))
    return np.argmax(a3, axis = 1)


def grad_sigmoid(z):
    #g'(z) =  g(z) .* (1-g(z)) ..element multiplication
...
def error(X,Y,theta1,theta2,reg):
    ...
    z2 = a1 @ theta1.T
    a2 = sigmoid(z2)    #hidden layer
    a2 = np.hstack((ones,a2))  #addierror(X,Y,theta2,theta1ng bias to hidden layer for 5000 training sets
